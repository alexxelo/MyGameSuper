package com.first.engine2.game.state
...
import com.first.engine2.game.result.RequestResult
...
/*
  ...
  fun useTip (gameRequest: GameRequest.TurnToAntimatter){
    activeNode = NodeAction(action = Action.ANTIMATTER, getIdAndInc())
  }
*/

  private fun createNewActiveNode(): Node {
    return if (Random.nextFloat() > 0.78f) {
      if (gameScore > 1000 && nodes.size > (nodes.size/2)) {
